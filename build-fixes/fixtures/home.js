...
var MAC={
    ...
    'Qrcode':function(){
        $('.mac_qrcode').attr('src','https://api.ijiexi.cc/barCode.php?url=' + MAC.Url);
    },
    'Image':{
        'Lazyload':{
            'Show': function(){
...
function isMobileOrTablet() {
    // 检查是否在浏览器环境（避免Node.js等非浏览器环境报错）
    if (typeof navigator === 'undefined' || !navigator.userAgent) {
        return false;
    }

    // 获取User-Agent并转为小写，统一匹配规则
    const userAgent = navigator.userAgent.toLowerCase();

    // 1. 手机特征正则（涵盖主流手机系统/设备关键词）
    const mobileRegex = /(iphone|ipod|android|ios|windows phone|mobile|phone|blackberry|webos|symbian|ucbrowser|opera mini|micromessenger)/i;
    
    // 2. 平板特征正则（涵盖主流平板设备关键词）
    const tabletRegex = /(ipad|tablet|playbook|kindle|nexus 7|nexus 10|galaxy tab|surface tablet)/i;

    // 核心判断：匹配手机 或 平板 任一规则即判定为移动设备
